...
namespace XmlTestAdapter
{
    ...
    public class XmlTestContainerDiscoverer : ITestContainerDiscoverer
    {
        ...
        private void OnSolutionProjectChanged(object sender, SolutionEventsListenerEventArgs e)
        {
            if (e != null)
            {
                var files = FindXmlFiles(e.Project);
        ...
        private IEnumerable<string> FindXmlFiles()
        {
            ...
            return loadedProjects.SelectMany(FindXmlFiles).ToList();
        }

        private IEnumerable<string> FindXmlFiles(IVsProject project)
        {
        ...
        {
            return ".xml".Equals(Path.GetExtension(path), StringComparison.OrdinalIgnoreCase);
        }
